...
public class ArgumentOutOfBoundsException<T> : ArgumentOutOfRangeException where T : struct,
    ...
    IFormattable
{
    public ArgumentOutOfBoundsException(ArgumentGuardType argumentGuardType, T referencedValue, string? paramName) :
        base(paramName, GetErrorMessage(argumentGuardType, referencedValue))
    {
    }

    private static string? GetErrorMessage(ArgumentGuardType argumentGuardType, T referencedValue)
    {
        return argumentGuardType switch
        {
